...
class RagModelFT:
    def __init__(self, plm) -> None:
        self.model = RagModel.from_pretrained_question_encoder_generator(
             plm, "facebook/bart-large"
        )
        self.model = self.model.eval()

    def generate(self, input_ids=None, attention_mask=None, context_input_ids=None, context_attention_mask=None, doc_scores=None, do_deduplication=False, num_return_sequences=1, num_beams=1, n_docs=None, **model_kwargs):
        
        if input_ids and context_input_ids is None:
            raise ValueError("Input_ids or context_input_ids must be provided.")

        if doc_scores is None:
            raise ValueError("doc_scores must be provided")
