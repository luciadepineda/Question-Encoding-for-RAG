...
import torch
from transformers import RagTokenizer, RagModel, RagRetriever, AutoTokenizer
...
def predict(query, ground_truth, docs, model, system, instruction, temperature, dataset):
    ...
    '''
    if (args.modelname == 'RagModelFT'):
        print("Model provided is a rag model")
        # Build tokenizer and retriever
        question_encoder_tokenizer = AutoTokenizer.from_pretrained("WhereIsAI/UAE-Large-V1")
        ...
        retriever = RagRetriever.from_pretrained(
            question_encoder_tokenizer, generator_tokenizer, index_name="exact", use_dummy_dataset=True
        )
        
        # Concatenate query and docs to form input_ids
        input_text = f"{query} {' '.join(docs)}"
        ...
        # Use the existing logic for other models
        print("Model provided is not a rag model")
    ...
    if '信息不足' in prediction or 'insufficient information' in prediction:
        ...
        # Replace with your actual checkanswer logic
...
if __name__ == '__main__':
    ...
    instances = []
    with open(f'/kaggle/input/rag-benchmarking/RGB-master/data/{args.dataset}.json','r') as f:
        for line in f:
    ...
    if args.factchecking:
        # prompt = yaml.load(open('/kaggle/input/rag-benchmarking/RGB-master/config/instruction_fact.yaml', 'r'), Loader=yaml.FullLoader)[args.dataset[:2]]
        prompt = yaml.load(open('/kaggle/input/rag-benchmarking/RGB-master/config/instruction_fact.yaml', 'r'), Loader=yaml.FullLoader)[args.dataset[:2]]
        resultpath = resultpath + '/fact'
    else:
        prompt = yaml.load(open('/kaggle/input/rag-benchmarking/RGB-master/config/instruction.yaml', 'r'), Loader=yaml.FullLoader)[args.dataset[:2]]
    ...
    elif 'RagModelFT' in modelname:
        model = RagModelFT(plm = args.plm)
    ...
    with open(filename,'w') as f:
        for instance in tqdm.tqdm(instances[:3]):
